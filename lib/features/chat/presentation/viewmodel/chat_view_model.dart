...
class ChatViewModel extends StateNotifier<ChatState> {
  ...
  Future<void> init(String receiverId) async {
    if (state.receiver?.userId == receiverId) {
      offSocket();
    }
    await initSocket();
  ...
  handleNewMessage(data) {
    ...
    showMySnackBar(message: message.sender?.fullName ?? "");
    state = state
  ...
  handleTyping() {
    ...
    socket.emit("typing",
        {'receiver': state.receiver?.userId!, 'sender': state.user?.userId});
  ...
  getMessages() async {
    ...
    if (hasReachedMax) {
      ...
      final data = await chatUseCase.getMessages(state.receiver!.userId!, page);
      data.fold(
        (failure) {
          state = state.copyWith(
            isLoading: false,
